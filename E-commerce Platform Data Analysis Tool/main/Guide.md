...
```
food_analyzer/
├── main.py                      # 🚀 统一入口（GUI + CLI双模式）
│
├── views/                       # 📄 页面层（7个核心页面）
│   ├── __init__.py              # 模块初始化
│   ├── dashboard.py             # 数据概览仪表盘
│   ├── data_collection.py       # 数据采集中心（实时登录模式）
│   ├── data_analysis.py         # 数据分析工作台
│   ├── report_center.py         # 报告中心（文件管理）
│   ├── cookie_management.py     # Cookie管理（实时登录说明页）
...
│   └── config.py                # 统一配置管理
│
├── components/                  # 🎨 组件层（UI组件库）
│   ├── __init__.py              # 组件导出
│   ├── loading.py               # 加载状态组件
│   └── toast.py                 # Toast通知组件
│
├── crawlers/                    # 🕷️ 爬虫引擎层
│   ├── selenium_base_crawler.py # Selenium基础引擎
│   ├── tmall_crawler_selenium.py# 天猫专用爬虫
│   └── scheduler.py             # 任务调度器
│
├── analyzers/                   # 🤖 AI分析层
...
┌─────────────────────────────────────────────┐
│              表示层 (Presentation)            │
│    views/* (7个页面模块) + components/*      │
├─────────────────────────────────────────────┤
│              业务逻辑层 (Business)            │
│    utils/* (工具集) + analyzers/* (AI分析)    │
├─────────────────────────────────────────────┤
│              数据访问层 (Data Access)         │
│    crawlers/* (爬虫) + database/* (数据库)    │
├─────────────────────────────────────────────┤
...
### Page 3: 报告中心 (`views/report_center.py`) ⭐重要更新
...
### Page 6: 使用帮助 (`views/help_guide.py`) ⭐已全面更新
...
| 场景 | 触发方式 | 处理方案 |
|-----|---------|---------|
| pending_navigation跳转 | 通过标记变量 | 增加 nav_version |
| 外部current_page变更 | 快捷操作按钮直接修改 | 增加 nav_version |
| 手动点击侧边栏 | 用户点击radio选项 | 正常更新 current_page |
...
2. **检查结构**: 确保 `views/` 和 `utils/` 目录完整
3. **简化测试**: 先用「测试模式」（10个商品）验证配置
4. **查看文档**: 本指南 + 各模块docstring注释
5. **参考帮助页**: 应用内的「使用帮助」页面有完整FAQ
